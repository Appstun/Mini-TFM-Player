...
export default class IcecastReadableStream {
  ...
  constructor(response, { onStream = noOp, ...rest }) {
    ...
    this._readableStream = new ReadableStream({
      async start(controller) {
        ...
        for await (const chunk of IcecastReadableStream.asyncIterator(response.body.getReader())) {
    ...
    this._reader = this._readableStream.getReader();
  ...
  /**
   * @returns The ReadableStreamDefaultReader instance
   */
  get Reader() {
    return this._reader;
  }
  ...
  async startReading() {
    try {
      for await (const i of IcecastReadableStream.asyncIterator(this._reader)) {
      }
    } catch (e) {
  ...
  /**
   * @description Wraps a ReadableStreamDefaultReader as an Async Iterator.
   * @param {ReadableStreamDefaultReader} reader ReadableStreamDefaultReader to convert to AsyncIterator
   * @returns {Symbol.asyncIterator} Async Iterator that wraps the ReadableStreamDefaultReader
   */
  static asyncIterator(reader) {
    return {
